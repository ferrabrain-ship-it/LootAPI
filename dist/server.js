...
import { asAddress, getAutoMine, getBuybacks, getCurrentRound, getLatestRoundTransition, getLeaderboardEarners, getLeaderboardMiners, getLeaderboardStakers, getRound, getRoundMiners, getRounds, getStakingStats, getStats, getTreasuryStats, getUserHistory, getUserRewards, getUserStake, } from './services/protocol.js';
...
app.get('/api/rounds', async (req) => {
    const { page = '1', limit = '12', lootpot } = req.query;
...
app.get('/api/user/:address/history', async (req) => {
    const { address } = req.params;
    const { type = 'deploy', limit = '100', roundId } = req.query;
    if (type !== 'deploy') {
...
app.get('/api/user/:address/events', async (_req, reply) => {
    startSse(reply);
    sendSse(reply, 'heartbeat', { timestamp: new Date().toISOString() });
