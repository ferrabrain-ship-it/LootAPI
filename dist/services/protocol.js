...
import { countSelectedBlocks, decodeBlockMask, etherString, safeAddressEq, toBigInt } from '../lib/format.js';
const DEPLOYED_EVENT = parseAbiItem('event Deployed(uint64 indexed roundId, address indexed user, uint256 amountPerBlock, uint256 blockMask, uint256 totalAmount)');
const DEPLOYED_FOR_EVENT = parseAbiItem('event DeployedFor(uint64 indexed roundId, address indexed user, address indexed executor, uint256 amountPerBlock, uint256 blockMask, uint256 totalAmount)');
const ROUND_SETTLED_EVENT = parseAbiItem('event RoundSettled(uint64 indexed roundId, uint8 winningBlock, address topMiner, uint256 totalWinnings, uint256 topMinerReward, uint256 lootpotAmount, bool isSplit, uint256 topMinerSeed, uint256 winnersDeployed)');
const GAME_STARTED_EVENT = parseAbiItem('event GameStarted(uint64 indexed roundId, uint256 startTime, uint256 endTime)');
const BUYBACK_EVENT = parseAbiItem('event BuybackExecuted(uint256 ethSpent, uint256 lootReceived, uint256 lootBurned, uint256 lootToStakers)');
...
const LOG_BLOCK_RANGE = 45000n;
let scanStartBlockPromise = null;
let protocolStatusCache = null;
...
async function contractExistsAt(address, blockNumber) {
    const code = await publicClient.getBytecode({ address, blockNumber });
    return !!code && code !== '0x';
}
async function findContractDeploymentBlock(address) {
    let low = 0n;
...
function getLogBlockNumber(log) {
...
function formatPricePayload(priceUsd, pairs) {
...
async function getRoundDeployLogs(roundId) {
    ...
    return [...direct, ...delegated].sort(compareLogsAsc);
}
function buildBlockStats(deployLogs) {
...
function computeUserDeployed(mask, amountPerBlock) {
    return BigInt(countSelectedBlocks(mask)) * amountPerBlock;
}
export async function getCurrentRound(user) {
...
function computeVaultedAmount(totalDeployed, winnersDeployed, totalWinnings) {
...
async function resolveRoundWinnerAddress(roundId, roundState, deployLogs) {
    if (roundState.isSplit || roundState.winnersDeployed === 0n)
        return null;
    if (roundState.topMiner !== '0x0000000000000000000000000000000000000001')
        return roundState.topMiner;
    ...
    for (const log of winningDeploys) {
        ...
        if (sample >= cumulative && sample < next)
            return log.address;
        cumulative = next;
    }
    return null;
...
async function getRoundState(roundId) {
    const [roundStruct, roundView, settledLog,] = await Promise.all([
        publicClient.readContract({
            address: CONTRACTS.gridMining,
            abi: gridMiningAbi,
            functionName: 'rounds',
...
export async function getRound(roundIdInput) {
...
export async function getRoundMiners(roundIdInput) {
    const roundId = BigInt(roundIdInput);
    const [roundState, deployLogs] = await Promise.all([
        getRoundState(roundId),
        getRoundDeployLogs(roundId),
...
export async function getBuybacks(page = 1, limit = 12) {
    ...
    const ordered = [...logs].sort((a, b) => compareLogsDesc(a, b));
...
export async function getUserStake(address) {
...
export async function getAutoMine(address) {
...
export async function getUserHistory(address, limit = 100, roundIdFilter) {
    const status = await getProtocolStatus();
    if (!status.gameStarted || status.currentRoundId === 0n) {
        return {
            history: [],
    ...
    const ordered = [...deploys, ...delegated].sort((a, b) => compareLogsDesc(a, b)).slice(0, limit);
    ...
    const totals = history.reduce((acc, entry) => {
        ...
        if (entry.roundResult.wonWinningBlock)
            acc.roundsWon += 1;
        return acc;
    }, {
        totalETHWon: 0n,
